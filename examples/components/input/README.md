...
```
So now when the template went rendered at window browser the input values will be 
bind with the controller class attributes. Any change at the inputs will be reflected in the controller attribute values. And to update the DOM when the controller attribute change use:
...
In compile time all controllers are injected with a attribute named 'elements' it contains all DOM elements attached to the template. So get the element by the 'akumos-name' data attribute, in the above example the 'lastName'. 

To call 'update()' method we use the code beloow:

```html

<button data-akumos data-type='method' data-event-click='obj.update()'></button>
  
```

The button above is a Akumos component, using the 'data-akumos' data attribute, the component type is 'method', defined using 'data-type'. And the 'data-event-*' is a listener to 'click' event at DOM component.

To call the controller use 'obj' reference and then call 'update()' method.

The input values goes be changed.
